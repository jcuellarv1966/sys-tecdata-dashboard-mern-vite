...
const ClientContractTypeEditScreen = () => {
  const navigate = useNavigate();
  const params = useParams(); // /clientContractType/:id
  const { id: clientContractTypeId } = params;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(`http://localhost:5000/api/clientcontracttypes/${clientContractTypeId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:5000/api/clientcontracttypes/${clientContractTypeId}`,
        { values }, { headers: { Authorization: `Bearer ${userInfo.token}` }, }
      );
      dispatch({ type: "UPDATE_SUCCESS" });
      toast.success('Client Contract Type updated successfully');
      navigate('/advisory/clientcontracttypes');
    } catch (err) {
  ...
  return (
    <div className="grid md:grid-cols-8 md:gap-0 mt-2">
      <Helmet>
        <title>Tec Data - Edit Client Contract Type ${clientContractTypeId}</title>
      </Helmet>
      <div>
        <SideNavbarAdvisory />
      </div>
        ...
        {loading ? (
          ...
          <div className='overflow-x-hidden'>
            ...
            <form className="mx-auto max-w-screen-md overflow-x-hidden" onSubmit={handleSubmit}>
              <h1 className='text-xl font-bold text-blue-600'>Edit Client Contract Type {clientContractTypeId}</h1>
              ...
              <div className="flex flex-wrap mt-3 mb-1">
                ...
                <div className="w-full md:w-1/2 mb-2 px-[2px] md:mb-0">
                  <Link to={`/advisory/clientcontracttypes`} className="inline-flex items-center h-9 py-0 px-4 text-sm font-medium text-center w-full rounded-md outline outline-offset-0 outline-1">
                    <FontAwesomeIcon
                      icon={faArrowLeft}
...
export default ClientContractTypeEditScreen;