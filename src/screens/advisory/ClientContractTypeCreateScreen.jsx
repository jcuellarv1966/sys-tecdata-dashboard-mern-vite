...
const ClientContractTypeCreateScreen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:5000/api/clientcontracttypes`, { name, },
        {
          headers: { Authorization: `Bearer ${userInfo.token}` },
        });
      dispatch({ type: "CREATE_SUCCESS" });
      setValues(initialState);
      toast.success("Client Contract Type created successfully");
  ...
  return (
    <div className="grid md:grid-cols-8 md:gap-0 mt-2">
      <Helmet>
        <title>Tec Data - Create Client Contract Type</title>
      </Helmet>
      <div>
        <SideNavbarAdvisory />
      </div>
      <div className="overflow-x-auto md:col-span-7 ml-12">
        ...
        <form className="mx-auto max-w-screen-md overflow-x-hidden" onSubmit={handleSubmit}>
          <h1 className='text-xl font-bold text-blue-600'>Create Client Contract Type</h1>
...
export default ClientContractTypeCreateScreen;