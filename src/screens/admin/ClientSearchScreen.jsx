...
export default function ClientSearchScreen() {
    ...
    const [{ loading, error, clients, pages, countClients }, dispatch] =
        useReducer(reducer, {
            loading: true,
            error: '',
        });
        ...
        const fetchData = async () => {
            try {
                const { data } = await axios.get(
                    `http://localhost:5000/api/clients/search?page=${page}&query=${query}&credit=${credit}&current=${current}&order=${order}`
                );
    ...
    const getFilterUrl = (filter) => {
        ...
        return `/searchclient?query=${filterQuery}&credit=${filterCredit}&current=${filterCurrent}&order=${sortOrder}&page=${filterPage}`;
    };
        ...
        <div className='grid md:grid-cols-8 md:gap-[2px] mt-2'>
            ...
            <div className="overflow-x-hidden md:col-span-7 ml-4">
                {loading ? (
                    ...
                    <>
                        <h1 className='text-2xl text-blue-900 text-bold'>Search Client(s) ...</h1>
                        <div className="flex justify-between md:grid-cols-7 md:gap-1 mb-3">
                            <div className='md:col-span-4'>
                                <div className='mt-2'>
                                    {countClients === 0 ? 'No' : countClients} Results
                                    {query !== 'all' && ' : ' + query}
                                    ...
                                    {query !== 'all' ||
                                        credit !== 'all' ||
                                        current !== 'all' ? (
                                        <a className="cursor-pointer" onClick={() => navigate('/searchclient')}>
                                            <FontAwesomeIcon
                                                icon={faTimesCircle}
                        ...
                        {clients.length === 0 && (
                            <MessageBox>No Client(s) Found</MessageBox>
                        )}
                        <div>
                            <table className="min-w-full">
                                ...
                                </thead>
                                {
                                    <tbody>
                                        {clients.map((client) => (
                                            <tr key={client._id} className="border-b text-[13.2px]">
                                                <td className=" p-1 ">
