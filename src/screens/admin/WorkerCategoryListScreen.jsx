...
export default function WorkerCategoryListScreen() {
    ...
    const deleteHandler = async (categoryId) => {
        if (window.confirm('Are you sure to delete?')) {
            try {
                ...
                await axios.delete(`http://localhost:5000/api/workerscategories/${categoryId}`, {
    ...
    return (
        <div className='grid md:grid-cols-8 md:gap-1 mt-4'>
            <Helmet>
                <title>Tec Data - List of Worker Categories</title>
            </Helmet>
            <div>
                <SideNavbar />
            </div>
            <div className="overflow-x-auto md:col-span-7 ml-12">
                <div className="flex justify-between mb-4">
                    <h1 className='text-2xl text-blue-900 text-bold'>List of Worker Categories</h1>
                    <div>
                        <Link to="/admin/workercategorycreate" className="inline-flex items-center h-9 py-0 px-4 text-sm font-medium text-center text-white bg-blue-700 rounded-md hover:bg-blue-800 hover:text-yellow-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mr-2">
                            <FontAwesomeIcon
                                icon={faUsers}
                                ...
                                {categories.map((category) => (
                                    <tr key={category._id} className="border-b">
                                        ...
                                        <td className=" p-1 px-2 w-14">
                                            <Link to={`/admin/workercategory/${category._id}`}>
                                                <FontAwesomeIcon
                                                    icon={faPencilAlt}
