...
export default function PartnerListScreen() {
    const [{ loading, error, partners, pages, loadingCreate, loadingDelete, successDelete }, dispatch] =
        useReducer(reducer, {
            loading: true,
            partners: [],
            error: '',
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const { data } = await axios.get(`http://localhost:5000/api/partners/admin?page=${page} `, {
    ...
    const deleteHandler = async (partnerId, images) => {
            ...
            try {
                ...
                if (images) {
                    for (let i = 0; i < images.length; i++) {
                        ...
                        await axios.post("http://localhost:5000/api/partners/removeimage", { file, },
                            { headers: { Authorization: `Bearer ${userInfo.token}` }, }
                        );
                    }
                };
    ...
    return (
        <div className='grid md:grid-cols-8 md:gap-1 mt-4'>
            <Helmet>
                <title>Tec Data - List of Partners</title>
            </Helmet>
            <div>
                <SideNavbar />
            </div>
            <div className="overflow-x-auto md:col-span-7 ml-12">
                <div className="flex justify-between mb-4">
                    <h1 className='text-2xl text-blue-900 text-bold'>List of Partners</h1>
                    <SearchPartnerBox />
                    <div>
                        <Link to="/admin/partnercreate" className="inline-flex items-center h-9 py-0 px-4 text-sm font-medium text-center text-white bg-blue-700 rounded-md hover:bg-blue-800 hover:text-yellow-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mr-2">
                            <FontAwesomeIcon
                                icon={faUserPlus}
                                    ...
                                    <tbody>
                                        {partners.map((partner) => (
                                            <tr key={partner._id} className="border-b">
                                                ...
                                                <td className=" p-1 px-0 text-right">{formatoMexico(parseFloat(partner.credit).toFixed(2))}</td>
