...
export default function ProductCategoryListScreen() {
    ...
    const deleteHandler = async (categoryId, images) => {
            ...
            try {
                ...
                if (images) {
                    for (let i = 0; i < images.length; i++) {
                        ...
                        await axios.post("http://localhost:5000/api/productscategories/removeimage", {
                            file,
                        });
                    }
                }
                await axios.delete(`http://localhost:5000/api/productscategories/${categoryId}`, {
    ...
    return (
        <div className='grid md:grid-cols-8 md:gap-1 mt-4'>
            <Helmet>
                <title>Tec Data - List of Product Categories</title>
            </Helmet>
            <div>
                <SideNavbar />
            </div>
            <div className="overflow-x-auto md:col-span-7 ml-12">
                <div className="flex justify-between mb-4">
                    <h1 className='text-2xl text-blue-900 text-bold'>List of Product Categories</h1>
                    <div>
                        <Link to="/admin/productcategorycreate" className="inline-flex items-center h-9 py-0 px-4 text-sm font-medium text-center text-white bg-blue-700 rounded-md hover:bg-blue-800 hover:text-yellow-200 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mr-2">
                            <FontAwesomeIcon
                                icon={faFolderPlus}
                            ...
                            {categories.map((category) => (
                                <tr key={category._id} className="border-b">
                                    ...
                                    <td className=" p-1 px-2 w-14">
                                        <Link to={`/admin/productcategory/${category._id}`}>
                                            <FontAwesomeIcon
                                                icon={faPencilAlt}
                                    ...
                                    <td className=" p-1 text-center w-14">
                                        <button onClick={() => deleteHandler(category._id, category.images)}>
                                            <FontAwesomeIcon
                                                icon={faTrashAlt}
