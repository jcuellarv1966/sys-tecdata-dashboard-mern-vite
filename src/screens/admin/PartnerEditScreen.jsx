...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case 'UPDATE_FAIL':
            return { ...state, loadingUpdate: false, error: action.payloa };
...
export default function PartnerEditScreen() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const { data } = await axios.get(`http://localhost:5000/api/partners/${partnerId}`);
    ...
    return (
        <div className="grid md:grid-cols-8 md:gap-0 mt-2">
            <Helmet>
                <title>Tec Data - Edit Partner ${partnerId}</title>
            </Helmet>
            <div>
                <SideNavbar />
            </div>
                        ...
                        <div className="overflow-x-auto md:col-span-2">
                            <form className="mx-auto max-w-screen-lg" onSubmit={handleSubmit}>
                                ...
                                <div className="flex flex-wrap mt-3 mb-1">
                                    ...
                                    <div className="w-full md:w-1/2 px-1 mb-2 md:mb-0">
                                        <Link to={`/admin/partners`} className="inline-flex items-center h-9 py-0 px-4 text-sm font-medium text-center w-full rounded-md outline outline-offset-0 outline-1">
                                            <FontAwesomeIcon
                                                icon={faArrowLeft}
