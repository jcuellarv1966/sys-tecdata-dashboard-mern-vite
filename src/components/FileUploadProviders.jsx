...
import { generatePublicUrlProviders } from "../../utils/urlConfig";
...
export default function FileUploadProviders({ values, setValues }) {
          ...
          async () => {
            ...
            try {
              ...
              const res = await axios.post(
                "http://localhost:5000/api/providers/uploadimage",
                form,
                config
              );
  ...
  const handleImageRemove = async (file) => {
    try {
      ...
      const res = await axios.post("http://localhost:5000/api/providers/removeimage", {
        file,
      },
        {
          ...
          <>
            <div className="relative" key={values.images[tab].file}>
              <div className="flex items-center justify-center w-72 h-120 mx-2 overflow-hidden rounded-2xl">
                <img src={generatePublicUrlProviders(values.images[tab].file)} alt={values.images[tab].file}
            ...
            <div className="row mx-0" style={{ cursor: 'pointer' }} >
              ...
              {values.images.map((image, index) => (
                <img key={index} src={generatePublicUrlProviders(image.file)} alt={image.file}
