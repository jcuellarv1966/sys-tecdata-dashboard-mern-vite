...
export default function SearchClientProformBox() {
    const navigate = useNavigate();
    const [query, setQuery] = useState('');

    const submitHandler = (e) => {
        e.preventDefault();
        navigate(query ? `/advisory/clientproformsearch/?query=${query}` : '/advisory/clientproformsearch');
    };
    ...
    return (
        <form className="mx-auto hidden w-64 justify-center md:flex h-min mt-1" onSubmit={submitHandler}>
            <input
                ...
                placeholder="search client proforms..."
            ></input>
            <button
