...
import { generatePublicUrlWorkers } from "../../utils/urlConfig";
...
export default function FileUploadWorkers({ values, setValues }) {
          ...
          async () => {
            ...
            try {
              ...
              const res = await axios.post(
                "http://localhost:5000/api/workers/uploadimage",
                form,
                config,
              );
  ...
  const handleImageRemove = async (file) => {
    try {
      ...
      const res = await axios.post("http://localhost:5000/api/workers/removeimage", {
        file,
      },
        {
        ...
        {values.images.length > 0 ? (
          <>
            <div className="relative" key={values.images[tab].file}>
              <div className="flex items-center justify-center w-60 h-full mx-2 overflow-hidden rounded-2xl">
                <img src={generatePublicUrlWorkers(values.images[tab].file)} alt={values.images[tab].file}
            ...
            <div className="row mx-0" style={{ cursor: 'pointer' }} >
              ...
              {values.images.map((image, index) => (
                <img key={index} src={generatePublicUrlWorkers(image.file)} alt={image.file}
