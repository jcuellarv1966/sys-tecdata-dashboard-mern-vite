...
import { generatePublicUrlClients } from "../../utils/urlConfig";
...
export default function FileUploadClients({ values, setValues }) {
          ...
          async () => {
            ...
            try {
              ...
              const res = await axios.post(
                "http://localhost:5000/api/clients/uploadimage",
                form,
                config,
              );
  ...
  const handleImageRemove = async (file) => {
    try {
      ...
      const res = await axios.post("http://localhost:5000/api/clients/removeimage", {
        file,
      },
        {
  ...
  return (
    <>
          ...
          <>
            <div className="relative" key={values.images[tab].file}>
              <div className="flex items-center justify-center w-72 h-120 mx-2 overflow-hidden rounded-2xl">
                <img src={generatePublicUrlClients(values.images[tab].file)} alt={values.images[tab].file}
            ...
            <div className="row mx-0" style={{ cursor: 'pointer' }} >
              ...
              {values.images.map((image, index) => (
                <img key={index} src={generatePublicUrlClients(image.file)} alt={image.file}
      ...
      <div className="row mt-4 mb-2 mx-0 px-2 w-full">
